// pipeline ACCEDA con destino 120D, desplegando en linux 
            ...
            steps{
              // logstash {
               script{
                   ...
                   echo "--> ****** ${deployTarget}"
                   sleep(15)
                   if ((deployTarget == "Validacion") || (deployTarget == "Pre-produccion") || (deployTarget == "Produccion") || (deployTarget == "Hotfix")){
                   //def libscheckout = new divindes.libs.checkout.checkoutParams()
                   libscheckout.paramsDownload(pathWS, env.JOB_NAME)
            ...
            steps{
              // logstash {
               script{
                       ...
                       //  taskUrl = libsBuild.buildProyectAcceda(env.BRANCH_NAME,pathWS,environment_deploy)
                ...
                stage("Desarrollo") {
                   ...
                   script{
                     ...
                     ((deployTarget == 'Desarrollo') && (env.BRANCH_NAME.matches("RELEASE(.*)")))){
                         ...
                         echo "*************************************************"
                     def libsProcesoDespliegueExternosDESVAL = new divindes.libs.DesplieguesExternos.procesoDespliegueExternos()
                     libsProcesoDespliegueExternosDESVAL.execDespliegueExterno(pathWS, env.BRANCH_NAME, env.JOB_NAME,deployTarget)
                     }
                   }
                 }
                }
